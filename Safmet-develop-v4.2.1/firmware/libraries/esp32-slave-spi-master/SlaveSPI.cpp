...
esp_err_t SlaveSPI::begin(gpio_num_t so, gpio_num_t si, gpio_num_t sclk, gpio_num_t ss, size_t buffer_size, int (*callback)()) {
    ...
    tx_buffer       = (uint8_t *)heap_caps_malloc(max(max_buffer_size, (unsigned int)32), SPI_MALLOC_CAP);
    rx_buffer       = (uint8_t *)heap_caps_malloc(max(max_buffer_size, (unsigned int)32), SPI_MALLOC_CAP);
...
esp_err_t SlaveSPI::initTransmissionQueue() {
    ...
    int size = min(max_buffer_size, (unsigned int)output_stream.length());                  // NOT over the buffer's size.
...
void SlaveSPI::get_values()
{
	Serial.print("Received value : ");
	Serial.print(((char *)transaction->rx_buffer)[0]);
	Serial.print(((char *)transaction->rx_buffer)[1]);
	Serial.print(((char *)transaction->rx_buffer)[2]);
	Serial.print(((char *)transaction->rx_buffer)[3]);
	Serial.print(((char *)transaction->rx_buffer)[4]);
	Serial.print(((char *)transaction->rx_buffer)[5]);
	Serial.print(((char *)transaction->rx_buffer)[6]);
	Serial.println(((char *)transaction->rx_buffer)[7]);
}
