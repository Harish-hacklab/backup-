...
//#define CAN_DEBUG
#ifdef CAN_DEBUG
static HardwareSerial Software(0);
#endif
uint8_t SR_VALUE = NULL;
...
int ESP32SJA1000Class::readRE(int ADDR) // user defined 
{
	return readRegister(ADDR);
}

void ESP32SJA1000Class :: can_recovery(bool f_Reset) // user defined
{
	if((readRegister(REG_SR) & 0x80) != 0x80)
	{
		#ifdef CAN_DEBUG
		Software.println("BUS_ACTIVE");
		#endif
		return ;
	}
	
	else if(((readRegister(REG_SR) & 0x80) == 0x80) || (REG_ECC == 0xd9))
	 {
		#ifdef CAN_DEBUG
		 Software.println("BUS_OFF");
		 #endif
		 writeRegister(REG_MOD, 0x01); // reset mode
		 writeRegister(REG_ECC,0x00);
		 writeRegister(REG_TXERR,0);
		 writeRegister(REG_RXERR,0);
		 writeRegister(REG_MOD,0x00);
		 #ifdef CAN_DEBUG
		 Software.print("MOD_VALUE : ");Software.println(readRegister(REG_MOD),HEX);
		 //Software.print("ECC_VALUE : ");Software.println(readRegister(REG_ECC),HEX);
		 Software.print("TXERR_VALUE : ");Software.println(readRegister(REG_TXERR),HEX);
		 Software.print("RXERR_VALUE : ");Software.println(readRegister(REG_RXERR),HEX);
		 #endif
	 }
	 else if((readRegister(REG_MOD) & 0x10) == 0x10)
	 {
		#ifdef CAN_DEBUG
		 Software.println("CAN_SLEEP");
		 #endif
		 writeRegister(REG_MOD,0x00);
	 }
	 //Software.print("ECC_VALUE : ");Software.println(readRegister(REG_ECC),HEX);
	...
	writeRegister(REG_MOD,0x00);	 
	#ifdef CAN_DEBUG
	 Software.print("SR_VALUE : ");Software.println(readRegister(REG_SR),HEX);
	 #endif
}
...
uint32_t last_start_time = millis();
  if (!CANControllerClass::endPacket()) {
	...
	#ifdef CAN_DEBUG
	Software.print("CURRENT_TX_STATUS_START : ");Software.println(readRegister(REG_SR),HEX);
	#endif
  // wait for TX buffer to free
  while ((readRegister(REG_SR) & 0x04) != 0x04) {
	 if((readRegister(REG_SR) & 0x80) == 0x80) // bus of recovery
	 {
		 #ifdef CAN_DEBUG
		 Software.println("BUS_OFF");
		 #endif
	 ...
	 if((readRegister(REG_MOD) & 0x10) == 0x10)
	 {
		#ifdef CAN_DEBUG
		 Software.println("CAN_SLEEP");
		 #endif
	 ...
	 if(millis() - last_start_time > 300 && ((readRegister(REG_SR)&0x04) == 0x04))
	 {
		 writeRegister(REG_CMR,0x02); 
		 #ifdef CAN_DEBUG
		 Software.println("aborting in start");
		 Software.print("SR_VALUE : ");Software.println(SR_VALUE,HEX);
		 #endif
		 //return 0x95;
	 }
  }
  ...
  while ((readRegister(REG_SR) & 0x08) != 0x08) {
	  #ifdef CAN_DEBUG
	  Software.print("CURRENT_TX_STATUS_END: ");Software.println(readRegister(REG_SR),HEX);
	  #endif
    if (readRegister(REG_ECC) == 0xd9) {
      modifyRegister(REG_CMR, 0x1f, 0x02); // error, abort
      return 0x93;
    }
	if((readRegister(REG_SR) & 0x10)==0x10 && (millis() - last_start_time > 200))
	{
		//writeRegister(REG_CMR,0x04); // release RX_BUFFER
		modifyRegister(REG_CMR, 0x1f, 0x02); //abort
		#ifdef CAN_DEBUG
		Software.println("ABORTED");
		#endif
		//return 0;
	}
	if((readRegister(REG_SR) & 0x80)==0x80) // bus off condition 
	{
		#ifdef CAN_DEBUG
		Software.println("STILL_BUS_OFF");
		#endif
  ...
  #ifdef CAN_DEBUG
  Software.print("CURRENT_TX_STATUS_TER: ");Software.println(readRegister(REG_SR),HEX);
  #endif
