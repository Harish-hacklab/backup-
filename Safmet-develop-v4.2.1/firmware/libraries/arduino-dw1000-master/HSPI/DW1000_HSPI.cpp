...
boolean    DW1000Class::_smartPower          = true;
...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	...
	SPI2.begin();
#ifndef ESP32
	SPI2.usingInterrupt(digitalPinToInterrupt(irq)); // not every board support this, e.g. ESP8266
#endif
...
void DW1000Class::deepSleep() {
	...
	memset(aon_wcfg, 0, LEN_AON_WCFG);    //memset ( void * ptr, int value, size_t num );
	readBytes(AON, AON_WCFG_SUB, aon_wcfg, LEN_AON_WCFG); // register address  = AON ; aon_wcfg = data array to be read into ;  LEN_AONWCFG = number of bytes expected to be received
	...
	readBytes(PMSC, PMSC_CTRL1_SUB, pmsc_ctrl1, LEN_PMSC_CTRL1);     //pmsc_ctrl1_sub = 0x04
	setBit(pmsc_ctrl1, LEN_PMSC_CTRL1, ATXSLP_BIT, false);           //ATXSLP_BIT - If this bit is set then the DW1000 will automatically transition into SLEEP or DEEPSLEEP mode after transmission of a frame has completed so long as thereare no unmasked interrupts pending.
	setBit(pmsc_ctrl1, LEN_PMSC_CTRL1, ARXSLP_BIT, false);        //ARXSLP_BIT - if set it will transition back to sleep mode after a receive attempt , if not set it will come out of sleep.
	...
	setBit(aon_cfg0, LEN_AON_CFG0, WAKE_SPI_BIT, true);  // setting this bit means the SPI SS pin can wake up the decawave
	setBit(aon_cfg0, LEN_AON_CFG0, WAKE_PIN_BIT, true);// setting this bit means you are telling the decawave that any of the 3 wakeup events can wakeup the decawave from sleep
	setBit(aon_cfg0, LEN_AON_CFG0, WAKE_CNT_BIT, false);  //setting this to zero means the sleep counter cannot wake the decawave from sleep , we want only the spi interrupt to wake it up
	setBit(aon_cfg0, LEN_AON_CFG0, SLEEP_EN_BIT, true);   // This bit enable the sleep mode , it has to be complemented with the UPL_CFG_BIT of the AON_CFG0 and AON_CFG1 register to upload the bit onto AON REGISTER
	...
	setBit(aon_ctrl, LEN_AON_CTRL, UPL_CFG_BIT, true);   // uploads the configurations of aon_cfg0 an d aon_cfg1 to the AON register
...
void DW1000Class::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	...
	SPI2.beginTransaction(*_currentSPI);
	digitalWrite(_ss, LOW);
	for(i = 0; i < headerLen; i++) {
		SPI2.transfer(header[i]); // send header
	}
	for(i = 0; i < n; i++) {
		data[i] = SPI2.transfer(JUNK); // read values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
	SPI2.endTransaction();
...
void DW1000Class::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	...
	for(i = 0; i < data_size; i++) {
		SPI2.transfer(data[i]); // write values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
	SPI2.endTransaction();
